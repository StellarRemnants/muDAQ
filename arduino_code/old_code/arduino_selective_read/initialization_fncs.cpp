...
const int DEVICE = UNO;           // Select device (Used to determine valid pin ranges)
...
/*
 * Get new baudrate by integer
 */
int get_baudrate(int delay_duration) {
  int serial_read_byte = 0;
  while (serial_read_byte == 0) {
    wait_for_serial(delay_duration);
    serial_read_byte = Serial.read();
//    Serial.print(serial_read_byte);
//    Serial.print(":");
//    Serial.print(NUM_ALLOWED_BAUDRATES);
//    Serial.print(":");
//    Serial.print(sizeof(ALLOWED_BAUDRATES));
//    Serial.print(":");
//    for (int i = 0; i < NUM_ALLOWED_BAUDRATES; i++) {
//      Serial.print(ALLOWED_BAUDRATES[i]);
//      Serial.print(",");
//    }
//    Serial.println("");
    delay(delay_duration);
    
    if (serial_read_byte >= NUM_ALLOWED_BAUDRATES) {
      Serial.write(0xFD);
      serial_read_byte = 0;
    }
    else {
      int baud_index = serial_read_byte;
      unsigned long baudrate = ALLOWED_BAUDRATES[serial_read_byte];
//      Serial.println(baudrate);
//      Serial.println("");
      Serial.write(serial_read_byte);
      delay(1000);
      Serial.flush();
      ...
      while (reconnect == 0) {
        wait_for_serial(delay_duration);
        serial_read_byte = Serial.read();
        if (serial_read_byte == baud_index) {
          reconnect = 1;
        }
      }
...
bool on_valid_init_byte(bool &initial_loop, int delay_duration, int *&pin_array, 
  ...
  Serial.write(0);            // Respond 0x00 to indicate succesful contact
  ...
  for (int i=0; i<num_pins; i++) { // Read in pins to be read from
    ...
    while (!valid_pin) {
      ...
      if (!valid_pin) {
        Serial.write(0xFE); // Respond 0xFE for invalid pin
      }
    }
  ...
  int baudrate = get_baudrate(delay_duration);
